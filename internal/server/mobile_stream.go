...
func (srv *Server) handleMobileEvents(w http.ResponseWriter, r *http.Request) {
	...
	// Asked of the writer, not answered by flushing it: a flush sends the
	// response head, and every header set after it — the content type, and
	// X-Accel-Buffering that keeps nginx from holding the stream back — never
	// reached the client. Through the frontend's nginx not a byte arrived.
	if !canFlush(w) {
		slog.Error("mobile_stream_cannot_flush")
	...
	if err := rc.Flush(); err != nil {
		slog.Error("mobile_stream_flush_failed", "error", err)
		return
	}
...
// canFlush reports whether w, or a writer it wraps, can flush — the question
// http.ResponseController answers, asked without flushing: the access-log
// middleware wraps the writer, and Unwrap is how the controller gets through.
