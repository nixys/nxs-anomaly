...
| `0031_chatops_status_replies` | Сохранённые ответы ChatOps `status` приводятся к виду 1.9.2: не больше 50 групп как id/title/severity/status плюс `open_count` и `truncated`. Раньше ответ нёс все открытые группы целиком, с логами, — мегабайты на вызов. Только данные, только ответы старого вида; место возвращается PostgreSQL после vacuum. |
| `0032_alert_groups_chain_idx` | Частичный индекс по `escalation_chain_id` незакрытых групп. Подключённый телефон на каждом тике потока спрашивает, какие открытые группы касаются его владельца; теперь это спрашивается по цепочкам, которые его называют, и индекс делает это поиском. Только индекс, откат оставляет его неиспользуемым. |
| `0033_alert_groups_recent_unresolved_idx` | Частичный индекс `(last_received_at DESC, id)` незакрытых групп. Команды чата `status` и `alerts` теперь считают и листают открытые группы в базе, свежие сначала, а не загружают их все; индекс задаёт этот порядок. Только индекс. |
