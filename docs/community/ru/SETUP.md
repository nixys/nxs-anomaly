...
| `NXS_ANOMALY_TRUSTED_PROXIES` | — (в Helm-чарте приватные диапазоны) | CIDR прокси, чьему `X-Forwarded-For` верить. Клиент — ближайший адрес вне этих диапазонов, так что подставить свой адрес нельзя. Лимит попыток входа считается по адресу клиента: за недоверенным прокси он общий для всех, и несколько неверных паролей блокируют вход всем |
